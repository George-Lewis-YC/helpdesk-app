...
def test_dashboard_user_logged_in(client, app):
...
def test_submit_ticket_logged_in(client, app):
    with app.test_request_context():
        with client.session_transaction() as sess:
            sess['username'] = 'testuser'
            sess['role'] = 'User'
...
def test_dashboard_admin_logged_in(client, app):
    with app.test_request_context():
        with client.session_transaction() as sess:
...
def test_dashboard_it_logged_in(client, app):
    with app.test_request_context():
        with client.session_transaction() as sess:
